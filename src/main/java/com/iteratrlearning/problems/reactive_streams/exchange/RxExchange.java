...
public class RxExchange
{
    ...
    private final PublishProcessor<MarketDataSnapshot> mak = PublishProcessor.create();
    private final PublishProcessor<ExecutionReport> execREp = PublishProcessor.create();
    ...
    private void updateMarketDataFeed()
    {
        ...
        mak.onNext(new MarketDataSnapshot(highestBid(), lowestAsk()));
    }
        ...
        final Order order, final Side side, final Order other)
    {
        ...
        execREp.onNext(new ExecutionReport(ask.getPrice(), bid.getParty(), ask.getParty()));
    }
